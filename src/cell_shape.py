...
class LineShape:
    ...
    def draw(self, start_position, end_position) -> None:
        draw.aaline(
            self.surface, self.color, start_position, end_position, CELL_SIZE // 4
        )
...
class PoligonShape:
    ...
    def draw(self, surface, color, position, size):
        ...
        for angle in range(0, 360, 360 // self.vertices_number):
            ...
            p = (
                x + (size // 2) * math.cos(angle_radians),
                y + (size // 2) * math.sin(angle_radians),
            )
            points.append(p)
...
class PoligonShapeAdapter(CellShape):
    def __init__(
        self, surface: Surface, color: tuple, poligon_shape: PoligonShape
    ) -> None:
        super().__init__(surface, color)
